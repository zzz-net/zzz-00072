...
router.post('/:id/resolve', (req: Request, res: Response) => {
  const { reason, result } = req.body as { reason: string; result: ManualResult };
  ...
  const tx = db.transaction(() => {
    db.prepare(`
      UPDATE anomalies SET status = 'resolved', manual_reason = ?, manual_result = ?, resolved_at = ? WHERE id = ?
    `).run(reason, result, now, req.params.id);
    ...
    `).run(historyId, req.params.id, reason, result, now);
  });
  tx();
